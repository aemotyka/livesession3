...
**Name (Team Member 1):** _Alex Motyka_  
...
**Enter answer here**
Work: O(n), because the reduce function adds up n entries.
Span: O(logn), because reduce is parallelized, and recursively divides input into two halves. The depth of this recursion is log n.
...
**Enter answer here**
First of all, nested for loops are relatively innefficient and hard to interpret. Second of all, this implementation is inherently sequential, because each operation of the for loop is dependant on the previous one. Given long enough inputs, the growth of the work (and span) would be asymptotically dominated by those of the parallelized map-reduce approach, which is much more efficient.
